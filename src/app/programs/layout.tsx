...
export default function ProgramsLayout({ children }: { children: React.ReactNode }) {
  ...
  const activeProgram =
    academicPrograms.find((p) => pathname === `/programs/${p.slug}`) ?? academicPrograms[0];
        ...
        <ScrollReveal className="relative bg-white border border-gray-200/90 rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="relative z-10 p-10 md:p-14 text-center max-w-3xl mx-auto space-y-5">
            ...
            <p className="font-sans text-sm sm:text-base leading-relaxed text-gray-600">
              Every day, students in <strong>{activeProgram.name}</strong> build research portfolios, secure corporate internships, and complete hackathons. Meet outstanding student achievers in this concentration.
            </p>
            <div className="pt-2">
              <Link
                href={`/spotlight?major=${encodeURIComponent(activeProgram.name)}`}
              ...
              >
                Meet {activeProgram.name} Students <ArrowRight className="h-5 w-5 stroke-[2.5]" />
              </Link>
            </div>
          </div>
        </ScrollReveal>
